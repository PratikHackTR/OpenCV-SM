...
STEPS = [
    ("neutral", "1 / 7 · Nötr duruş", "Kameraya düz bak. Başın ve omuzların görünsün. Rahat dur ve 1,2 saniye kıpırdama."),
    ("left", "2 / 7 · Sola bak", "Gövdeni sabit tut. Başını kameranın dönmesini istediğin kadar sola çevir; yüzünü tamamen gizleme."),
    ("right", "3 / 7 · Sağa bak", "Gövdeni sabit tut. Başını belirgin biçimde sağa çevir ve bu konumda tut."),
    ("swing", "4 / 7 · Yukarı ağ", "Bir elini omuz üstüne kaldır. İşaret ve serçe parmak yukarı, orta ve yüzük parmak kapalı olsun."),
    ("aim", "5 / 7 · İki elle nişan", "İki elini birbirinden ayır. Kameraya doğru, rahat göğüs/karın hizanda iki ağ işareti yap. İki el de tamamen görünsün."),
    ("open", "6 / 7 · Açık el", "İki avucunu nişan yüksekliğinde aç; dört parmağın da açık olsun. Bu örnek E hareketini ayırır."),
    ("fist", "7 / 7 · Kapalı el", "Aynı yükseklikte iki elini yumruk yap. Sıkmadan sabit tut. Tamamlanınca profil kaydedilir."),
]
...
class GuidedCalibration:
    ...
    def update(self, observation, now):
        ...
        if step in ("neutral", "left", "right") and max(v["head_yaw"] for v in values) - min(v["head_yaw"] for v in values) > 0.12:
            self.samples.clear()
            self.progress = 0
            self.message = "Baş açısı değişiyor; bu konumu sabit tut."
            return
    ...
    def validate_observation(self, step, obs):
        ...
        if step in ("neutral", "left", "right") and obs["head_yaw"] is None:
            return "Baş açısı ölçülemiyor; yüz ve kulak/göz noktaları görünmeli."
        if step in ("left", "right"):
            neutral = median(v["head_yaw"] for v in self.collected["neutral"])
            delta = obs["head_yaw"] - neutral
            if abs(delta) < 0.12:
                return "Başını biraz daha belirgin çevir."
            if step == "right":
                left = median(v["head_yaw"] for v in self.collected["left"]) - neutral
                if left * delta >= 0:
                    return "Önceki adımdan ters yöne bakmalısın."
    ...
    def build_profile(self):
        ...
        profile = {"version": 1, "neutral": {k: median(v[k] for v in neutral)
                   for k in ("y", "center", "scale", "yaw", "head", "head_yaw")}}
        zero = profile["neutral"]["head_yaw"]
        profile["head_left"] = median(v["head_yaw"] for v in self.collected["left"]) - zero
        profile["head_right"] = median(v["head_yaw"] for v in self.collected["right"]) - zero
        profile["head_gate"] = min(0.75, max(0.50, pstdev(v["head_yaw"] for v in neutral) * 4 /
                                             min(abs(profile["head_left"]), abs(profile["head_right"]))))
        templates = {}
        ...
        for index, web in enumerate(templates["web"]):
            if any(dist(web, other) < 0.22 for kind in ("open", "fist") for other in templates[kind]):
                self.index = 3 if index == 0 else 4
