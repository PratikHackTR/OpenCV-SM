...
class Window(QMainWindow):
    ...
    def show_guide(self):
        QMessageBox.information(self, "Hareket rehberi",
            ...
            "KAMERA: Başını belirgin şekilde sağa/sola çevirip kısa süre tut. Nötr alandaki küçük hareketler kamerayı oynatmaz. Gövde yalnızca A/D yönünü kontrol eder.\n\n"
            "NİŞAN: İki elini ayırarak göğüs/karın hizasında kameraya ağ işareti yap. Sağ tık basılı kalır. Elleri indirerek çık. Kalibrasyon senin el şeklini ve nişan yüksekliğini öğrenir.\n\n"
    ...
    def tick(self):
        ...
        self.debug_status.setText(f"SWING: {debug.get('swing_reason', '—')}\nNİŞAN: {debug.get('aim_reason', '—')}\n"
                                 f"BAŞ: {debug.get('head_reason', '—')} · değer {debug.get('head_value', 0):+.2f} / eşik {debug.get('head_gate', .55):.2f}\n{hands_text}")
