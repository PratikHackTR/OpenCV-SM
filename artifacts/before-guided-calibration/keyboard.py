...
class InputGuard(threading.Thread):
    ...
    def step(self, now):
        with self.lock:
            dt = min(max(now - self.last_step, 0), 0.04) if self.last_step is not None else 0
            self.last_step = now
            ...
            self.key(self.SHIFT, decision.swing and not decision.aim)
            if "jump" in pending and "dodge" not in pending:
            ...
            left = not decision.aim and steer < (-0.18 if self.A in self.held else -0.30)
            right = not decision.aim and steer > (0.18 if self.D in self.held else 0.30)
            if not left:
            ...
            for i, value in enumerate((steer, decision.look_y)):
