...
class Window(QMainWindow):
    ...
    def show_guide(self):
        QMessageBox.information(self, "Hareket rehberi",
            ...
            "SHIFT: Ağ işaretini omuz üstüne kaldır; işaret ve serçe parmak yukarı baksın. Elini indirince veya açınca bırakır. Yumruk swing yapmaz.\n\n"
            "YÖN: Gövdeni sağa/sola eğ veya çevir. A/D ve kamera birlikte yönlenir; swing sırasında da çalışır. Nötr duruşta durur.\n\n"
            "NİŞAN: İki elinle karın hizasında kameraya ağ işareti yap. Sağ tık basılı kalır. Nişanda gövde yalnızca kamerayı yönlendirir; baş açın dikey nişanı ayarlar. Elleri indirerek çık.\n\n"
    ...
    def tick(self):
        ...
        if self.smooth_preview.isChecked() and not self.isMinimized():
        ...
        if not self.isMinimized() and not self.smooth_preview.isChecked():
