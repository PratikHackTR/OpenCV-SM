...
class Window(QMainWindow):
    ...
    def build(self):
        ...
        side.addWidget(label("Nişan el hassasiyeti  ·  yavaş → hızlı", "muted"))
        ...
        side.addWidget(label("Hareket toleransı  ·  az → çok", "muted"))
        ...
        self.arm_steering = QCheckBox("Swing sırasında kolla sağ / sol yön ver")
        self.arm_steering.setChecked(True)
        side.addWidget(self.arm_steering)
        self.invert_steering = QCheckBox("Sağ / sol yönünü ters çevir")
    ...
    def show_guide(self):
        QMessageBox.information(self, "Hareket rehberi",
            ...
            "YÖN: Swing başlarken kolunun konumu nötr kabul edilir. Ağ elini belirgin biçimde yana taşı ve kısa süre tut; başlangıç konumuna getirince A/D bırakılır. Normal duruşta otomatik yürüme yok. Başını ve gövdeni rahatça oynatabilirsin.\n\n"
            "NİŞAN: İki elini ayırarak göğüs/karın hizasında kameraya ağ işareti yap. Sağ tık basılı kalır. İki elinin orta noktasını sağ/sol veya yukarı/aşağı taşı: kamera hareketini takip eder. Ellerin durunca kamera durur. Parmaklarını açıp kapatman bakışı değiştirmez. Elleri indirerek çık. Kalibrasyon senin el şeklini ve nişan yüksekliğini öğrenir.\n\n"
    ...
    def arm_changed(self, checked):
        ...
        calibrating = self.calibration_dialog and self.calibration_dialog.isVisible()
        if checked and (not self.pipeline or not target.lower().endswith(".exe") or calibrating):
            self.arm.setChecked(False)
            self.input_status.setText("Kamerayı açın, kalibrasyonu tamamlayın ve oyun .exe adını girin.")
            return
    ...
    def tick(self):
        ...
        calibrating = bool(self.calibration_dialog and self.calibration_dialog.isVisible())
        ...
        self.pipeline.arm_steering = self.arm_steering.isChecked()
