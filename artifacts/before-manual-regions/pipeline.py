...
class Pipeline:
    ...
    def infer(self):
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                gestures.arm_steering = self.arm_steering
    ...
    def draw(frame, hands, body, decision=None):
        ...
        if "aim_midpoint" in debug:
            mx, my = debug["aim_midpoint"]
            ax, ay = debug["aim_anchor"]
            slop = debug["aim_slop"]
            cv2.rectangle(frame, (int((ax-slop)*width), int((ay-slop)*height)),
                          (int((ax+slop)*width), int((ay+slop)*height)), (80, 230, 150), 2)
            cv2.drawMarker(frame, (int(mx*width), int(my*height)), (100, 240, 255), cv2.MARKER_CROSS, 22, 2)
        if "arm_neutral_x" in debug:
            for x in (debug["arm_neutral_x"] - debug["arm_slop"], debug["arm_neutral_x"] + debug["arm_slop"]):
                cv2.line(frame, (int(x*width), 60), (int(x*width), height), (80, 230, 150), 1)
        if decision:
            ...
            cv2.putText(frame, f"ARM {decision.steer:+.0f}   HAND DX {decision.aim_dx:+.2f} DY {decision.aim_dy:+.2f}",
