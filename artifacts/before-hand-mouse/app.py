...
from .regions_ui import RegionsDialog
from .regions import valid_regions
...
class Window(QMainWindow):
    ...
    def build(self):
        ...
        side.addWidget(label("Yukarı ağ → Space ardından Shift\nAğ eli kutuda: Sol A · Sağ D · Üst W\nİki el nişan kutusunda → Nişan\nBaş dönüşü → Kamera\nAç/kapa → E\nYumruk → Saldırı · Yüksel → Zıpla\nEğil/doğrul → Kaçın", "muted"))
        guide = QPushButton("Hareket rehberi")
        guide.clicked.connect(self.show_guide)
        side.addWidget(guide)
        self.calibrate = QPushButton("Adım adım kalibrasyon · 5 adım")
        self.calibrate.clicked.connect(self.reset_calibration)
        self.edit_regions = QPushButton("A / D / W ve nişan bölgelerini çiz")
        self.edit_regions.clicked.connect(self.open_regions)
        side.addWidget(self.calibrate)
        side.addWidget(self.edit_regions)
        ...
        side.addWidget(label("Baş ile kamera hızı  ·  yavaş → hızlı", "muted"))
        ...
        side.addWidget(label("Baş dönüş toleransı  ·  az → çok", "muted"))
        self.motion_tolerance = QSlider(Qt.Orientation.Horizontal)
        self.motion_tolerance.setRange(2, 10)
        self.motion_tolerance.setValue(5)
        side.addWidget(self.motion_tolerance)
        self.invert_steering = QCheckBox("Baş ile bakış yönünü ters çevir")
    ...
    def show_guide(self):
        QMessageBox.information(self, "Hareket rehberi",
            "Önce yüzün, omuzların ve mümkünse belin görünecek şekilde nötr duruşunu kalibre et.\n\n"
            "SWING: Ağ işaretini omuz üstüne kaldır; işaret ve serçe parmak yukarı baksın. Önce Space 100 ms basılır, bırakılır, sonra Shift tutulur. Elini indirince veya açınca bırakır. Aynı tutuş boyunca tekrar zıplamaz.\n\n"
            "YÖN: Kalibrasyon sonrası dört bölgeyi fareyle çiz. Sol=A, sağ=D, baş üstü=W. Ağ işaretindeki bileğini bir kutuda kısa süre tut; kutudan çıkınca tuş bırakılır. İki farklı yön aynı anda istenirse hareket durur.\n\n"
            "KAMERA: Başını belirgin biçimde sağa/sola çevirip kısa süre tut. Nötre dönünce durur. Nişandayken başını yukarı/aşağı eğerek dikey bakışı ayarla. Gövde konumu A/D/W üretmez.\n\n"
            "NİŞAN: Çene–omuz–göğüs arasına çizdiğin dikdörtgende iki elinle ağ işareti yap. Sağ tık tutulur. Eller bölgeden çıkınca bırakılır.\n\n"
            "E: Nişandayken bir elini aç, ardından tekrar ağ işareti veya yumruk yap. Her aç/kapa bir alet kullanımıdır.\n\n"
            "SALDIRI: Yumruğunu göğüs/karın hizasında hızlıca savur. Tek savuruş tek sol tıktır; tekrar saldırmak için eli kısa süre sakinleştir. Nişandayken saldırı tetiklenmez.\n\n"
    ...
    def stop_camera(self):
        if self.pipeline:
            self.pipeline.stop()
        if self.regions_dialog and self.regions_dialog.isVisible():
            self.regions_dialog.reject()
    ...
    def reset_calibration(self):
        if self.pipeline:
            ...
            self.debug_overlay.setChecked(True)
            self.calibration_dialog = CalibrationDialog(self.pipeline, self)
            self.calibration_dialog.finished.connect(self.after_calibration)
    ...
    def open_regions(self):
        if not self.pipeline or self.pipeline.stop_event.is_set():
            return
        if self.regions_dialog and self.regions_dialog.isVisible():
            self.regions_dialog.raise_()
            return
        if self.calibration_dialog and self.calibration_dialog.isVisible():
            return
        view = self.pipeline.views.get()
        if not view or not view.decision.calibrated:
            self.input_status.setText("Önce kamera karşısında nötr duruşunu kalibre et.")
            return
        self.arm.setChecked(False)
        self.guard.configure(False)
        self.debug_overlay.setChecked(True)
        self.regions_dialog = RegionsDialog(self.pipeline, self)
        self.regions_dialog.show()

    def editing_controls(self):
        return any(dialog and dialog.isVisible() for dialog in (self.calibration_dialog, self.regions_dialog))

    def arm_changed(self, checked):
        target = self.target.text().strip()
        calibrating = self.editing_controls()
        if checked and (not self.pipeline or not target.lower().endswith(".exe") or calibrating or not valid_regions(self.pipeline.regions)):
            self.arm.setChecked(False)
            self.input_status.setText("Kamerayı açın, kalibrasyonu ve dört bölgeyi tamamlayın; oyun .exe adını girin.")
            return
    ...
    def tick(self):
        ...
        self.edit_regions.setEnabled(bool(self.pipeline) and not calibrating)
        self.calibrate.setEnabled(bool(self.pipeline) and not (self.regions_dialog and self.regions_dialog.isVisible()))
        ...
        if self.pipeline.error:
            ...
            return
        self.pipeline.tolerance = self.motion_tolerance.value() / 100
        ...
        if self.smooth_preview.isChecked() and not self.debug_overlay.isChecked() and not self.isMinimized():
            ...
            if item is not None and item[0] != getattr(self, "preview_stamp", None):
                ...
                self.render_preview(cv2.flip(item[1], 1))
        ...
        self.debug_status.setText(f"SWING: {debug.get('swing_reason', '—')}\nNİŞAN: {debug.get('aim_reason', '—')}\n"
                                 f"YÖN: {debug.get('motion_reason', '—')} · BAŞ: {debug.get('head_reason', '—')}\n{hands_text}")
        self.progress.setValue(100 if view.decision.calibrated else round(view.decision.progress * 100))
        self.calibration_status.setText("Adım adım kalibrasyon sürüyor · girdiler kapalı" if calibrating else "Duruş hazır · hareket edebilirsin" if view.decision.calibrated else "Yüzün ve omuzların görünsün; 1,2 saniye sabit dur.")
