...
from .regions import contains, valid_regions
...
class Gestures:
    ...
    def update(self, hands, pose, now, threshold=0.085, handedness=None):
        ...
        regions_ready = valid_regions(self.regions)
        waist = regions_ready and len(hands) == 2 and all(contains(self.regions["aim"], h) for h in hands)
        result.debug.update(regions=self.regions, shoulder_y=shoulder_y)
        ...
        if self.aim:
            if not waist:
                ...
                self.aim_bad_since = None
        result.aim = self.aim
        result.debug["aim_reason"] = ("AKTİF: sağ tık" if self.aim else "Önce dört kontrol bölgesini çiz" if not regions_ready else "İki el bulunamadı" if len(hands) != 2
        ...
        # Region membership, not body lean or arm displacement, selects movement.
        candidate = None
        if regions_ready and not result.aim:
            hits = {name for name in ("left", "right", "forward")
                    if any(shape == "web" and contains(self.regions[name], hand)
                           for hand, shape in zip(hands, shapes))}
            if len(hits) == 1:
                candidate = hits.pop()
        if candidate != self.region_candidate:
            self.region_candidate, self.region_since = candidate, now
        if candidate is not None and self.region_since is not None and now - self.region_since >= .15:
            result.steer = -1 if candidate == "left" else 1 if candidate == "right" else 0
            result.forward = candidate == "forward"
        result.debug.update(active_region=candidate if result.steer or result.forward else None,
                            motion_reason="Bölge: " + (candidate or "nötr / çakışan eller"))
        ...
        gate = min(.85, max(.35, (self.profile["head_gate"] if self.profile else .55) + (self.tolerance - .05) * 4))
        ...
        pitch = (head - self.head_zero) * 2.5
        result.look_y = max(-1, min(1, pitch)) if self.aim and abs(pitch) > .3 else 0
