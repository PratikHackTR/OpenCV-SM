...
class InputGuard(threading.Thread):
    ...
    def publish_decision(self, decision, stamp):
        with self.lock:
            ...
            for action in ("jump", "dodge", "gadget", "attack", "web_attack", "swing_cast"):
    ...
    def step(self, now):
        with self.lock:
            ...
            if wants_swing and (not self.swing_active or "swing_cast" in pending):
            ...
            self.key(self.E, now < self.pulses.get("gadget", 0))
            ...
            self.key(self.W, decision.forward and not decision.aim)
            if decision.swing and not decision.aim:
                self.mouse_remainder[0] += decision.look_x * (-1 if self.invert_x else 1) * self.camera_speed * dt
                self.mouse_remainder[1] = 0
                if not decision.look_x:
                    self.mouse_remainder[0] = 0
            elif decision.mouse_session:
