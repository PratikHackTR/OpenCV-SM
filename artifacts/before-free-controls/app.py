...
class Window(QMainWindow):
    ...
    def show_guide(self):
        QMessageBox.information(self, "Hareket rehberi",
            ...
            "SWING: Ağ işaretini yukarıda tut: Space ardından Shift basılır. Elinin başlangıç konumu merkezdir. Eli sağa/sola taşıyınca kamera döner; merkeze getirince durur. İşareti bırakınca veya eli indirince Shift ve kamera kontrolü bırakılır. Yeni tutuş yeni merkez alır.\n\n"
            "NİŞAN: Yüz hizasında işaret ve orta parmağı yukarı uzat, diğerlerini kapat. Sağ tık tutulur; aynı elin avuç merkezini fare gibi taşı. El durunca bakış durur. Parmak şeklini değiştirerek eli kullanabilirsin. Tekrar iki parmak işareti yaparak veya eli bel altına indirerek çık.\n\n"
            "E: İki açık elini ayır, hızlıca birbirine yaklaştırarak alkış yap. Her alkış bir E; tekrar ayırarak hazırla.\n\n"
            "F: İki elinde işaret parmaklarını (istersen orta parmaklarla birlikte) yukarı uzat, kalanları kapat; parmak uçlarını birleştirip kısa süre tut. Her yeni işaret bir F.\n\n"
            "W: Yumruğunu kalbinin üstüne getirip kısa süre tut. Tuttuğun sürece ileri gider, çekince durur.\n\n"
            "SALDIRI: Nişan/swing/selam dışında yumruğunu hızlıca savur.\n\n"
    ...
    def arm_changed(self, checked):
        ...
        if checked and (not self.pipeline or not target.lower().endswith(".exe") or calibrating):
            self.arm.setChecked(False)
            self.input_status.setText("Kamerayı açın, kalibrasyonu tamamlayın; oyun .exe adını girin.")
            return
