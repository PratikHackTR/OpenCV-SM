...
class Window(QMainWindow):
    ...
    def build(self):
        ...
        self.resume_walk = QPushButton("Yürüyüşü yeniden aç")
        self.resume_walk.clicked.connect(lambda: self.guard.resume_movement())
        side.addWidget(self.resume_walk)
        self.walk_status = label("WASD hazır", "muted")
        side.addWidget(self.walk_status)
        ...
        side.addStretch()
        for widget in panel.findChildren(QPushButton) + panel.findChildren(QLineEdit):
            widget.setMinimumHeight(44)
        panel.ensurePolished()
        tabs.setMinimumHeight(max(game_page.sizeHint().height(), motion_page.sizeHint().height()) + 50)
        scroll = QScrollArea()
    ...
    def tick(self):
        locked = self.guard.movement_locked
        self.resume_walk.setEnabled(bool(self.pipeline) and locked and not self.guard.decision.swing and not self.guard.swing_active and self.guard.swing_exit_until is None)
        self.walk_status.setText("WASD kilitli" if locked else "WASD hazır")
