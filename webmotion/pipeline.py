...
class Pipeline:
    ...
    def infer(self):
        ...
        try:
            ...
            gestures = Gestures()
            movement_epoch = self.guard.movement_epoch
            movement_epoch = self.guard.movement_epoch
            ...
            while not self.stop_event.is_set():
                ...
                if reset_epoch != self.guard.reset_epoch:
                    ...
                    reset_epoch = self.guard.reset_epoch
                if movement_epoch != self.guard.movement_epoch:
                    gestures.hand_controls.resume_walking()
                    movement_epoch = self.guard.movement_epoch
                if self.guard.movement_locked:
                    gestures.hand_controls.walk_suspended = True
                if movement_epoch != self.guard.movement_epoch:
                    gestures.hand_controls.resume_walking()
                    movement_epoch = self.guard.movement_epoch
                if self.guard.movement_locked:
    ...
    def draw_movement(frame, decision):
        ...
        if decision.swing or decision.swing_exit:
            ...
            return
        if decision.walk_locked:
            cv2.putText(frame,"WASD KILITLI: YURUYUSU YENIDEN AC",(12,height-16),cv2.FONT_HERSHEY_SIMPLEX,.48,(100,210,255),1)
            return
        if decision.walk_locked:
            cv2.putText(frame,"WASD KILITLI: YURUYUSU YENIDEN AC",(12,height-16),cv2.FONT_HERSHEY_SIMPLEX,.48,(100,210,255),1)
            return
